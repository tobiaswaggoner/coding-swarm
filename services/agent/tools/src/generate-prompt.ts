...
import { loadBaseConfig, loadProjectManagerConfig, hasSupabase, type AgentRole } from "./config.js";
import { createDatabase, type Task, type Project, type TaskResult } from "./db/supabase.js";
...
async function loadProjectManagerContext(context: PromptContext): Promise<void> {
...
function loadDeveloperContext(context: PromptContext): void {
...
function loadSystemPrompt(role: AgentRole, runtimeDir: string): string {
...
function buildProjectContext(context: PromptContext): string | null {
  const parts: string[] = [];
...
function buildTaskSection(context: PromptContext): string {
...
function getProjectManagerTask(trigger: TriggerContext, hasConversation: boolean): string {
...
function loadFileContent(path: string): string | undefined {
  if (existsSync(path)) {
